<template>
    <div>
      ...
      <b-modal v-model="mostrarFormulario" :title="pessoa.id ? 'Editar Pessoa' : 'Adicionar Pessoa'">
        ...
        <b-form @submit.prevent="salvarPessoa">
          ...
          <b-button @click="cancelarEdicao">Cancelar</b-button>
        </b-form>
      </b-modal>
      ...
      <b-table :items="pessoas" :fields="campos">
        <template #cell(acoes)="data">
          <b-button size="sm" @click="editarPessoa(data.item)">Editar</b-button>
          <b-button size="sm" variant="danger" @click="excluirPessoa(data.item.id)">Excluir</b-button>
        </template>
      </b-table>
    </div>
  </template>
